...
const fetchAndSaveScores = async (url, key, strategy) => {
  try {
    const requestUrl = `${API_URL}?url=${encodeURIComponent(
      url
    )}&fields=lighthouseResult/categories/*/score&prettyPrint=false&strategy=${strategy}&category=performance&category=best-practices&category=accessibility&category=seo&key=${key}`;
  ...
  try {
    const embed = {
      ...
      fields: [
        {
          name: "Performance",
          value: scores.performance !== null ? scores.performance + " / 100" : "N/A",
          inline: false,
        },
        {
          name: "Accessibility",
          value: scores.accessibility !== null ? scores.accessibility + " / 100" : "N/A",
          inline: false,
        },
        {
          name: "Best Practices",
          value: scores["best-practices"] !== null ? scores["best-practices"] + " / 100" : "N/A",
          inline: false,
        },
